...
function LandingPage() {
  ...
  const getHotPost = async () => {
    try {
      ...
      if (response) {
        ...
        console.error(
          "Error fetching data: getHotPost response.data is undefined"
        );
      }
    } catch (error) {
  ...
  return (
    <div style={{ width: "100%" }}>
                  ...
                  {hotPost.map((item) => {
                    ...
                    if (item.postType === "PROJECT") {
                      ...
                      TagStyle = {
                        ...
                        padding: "0px 5px 0px 5px",
                        backgroundColor:
                          "lightyellow" /* 원하는 색상으로 변경 */,
                        borderRadius:
                          "50px" /* 타원형 모양을 만들기 위해 사용 */,
                    ...
                    return (
                      ...
                      >
                        <Card
                          onClick={() => onClickHandler(item.postType, item.id)}
                          size="small"
                          style={{
                            cursor: "pointer",
                            width: "95%",
                        ...
                        >
                          ...
                          <br />
                          <div>마감일 | {formatDate(item.endDate)}</div>
                          <div
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
      ...
      {isAuthenticated &&
        userRole === "ADMIN" && ( //인증되었고, 관리자만 볼 수 있는 화면
          // Row, Col : 그리드(창의 크기에 맞춘 반응형)를 위해 사용되는 애.
          ...
          <div>
            ...
            <div style={{ marginLeft: "15%", marginRight: "15%" }}>
              <Row gutter={[16, 16]}>
                ...
                >
                  <div
                    style={{ position: "absolute", zIndex: 10, width: "79%" }}
                  >
                    {renderSection("User", data.userSearchDtoList)}
                    {renderSection("Project", data.projectSearchDtoList)}
                    {renderSection("Study", data.studySearchDtoList)}

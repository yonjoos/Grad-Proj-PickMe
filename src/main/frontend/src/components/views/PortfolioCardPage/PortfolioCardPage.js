...
import {
  request,
  setSaveRecommendedList,
  setIsRecommededPortfolioView,
} from "../../../hoc/request";
import {
  saveRecommendedList,
  setRecommendPortfolioView,
...
function PortfolioCardPage() {
  ...
  const selectedRecommendedList = useSelector(
    (state) => state.recommend.recommendedList
  );
  const selectedIsRecommededPortfolioView = useSelector(
    (state) => state.recommend.isRecommededPortfolioView
  );
  ...
  useEffect(() => {
    console.log(
      "selectedIsRecommededPortfolioView : ",
      selectedIsRecommededPortfolioView
    );
    if (!selectedIsRecommededPortfolioView) {
      setCurrentPage(0);
  ...
  const toggleBanner = (banner) => {
    ...
    setIsRecommend(0);
    if (banner === "all") {
      // 만약 선택된 배너가 전체라면 selectedBanners: [all]
      setSelectedBanners(["all"]);
    } else if (selectedBanners.includes("all")) {
      // 만약 '전체' 상태에서 '전체'가 아닌 다른 버튼을 눌렀다면, [all] -> [특정 배너]
      ...
      const allBannersUnselected = !["web", "app", "game", "ai"].some((b) =>
        updatedBanners.includes(b)
      ); // 모든 배너가 제거되어있으면 true , 하나라도 배너가 활성화되어있으면 false
  ...
  const renderContent = () => {
    if (showRecommend === 1) {
      ...
      if (data === "") {
        alert(
          "추천 기능을 사용하시려면, 정확한 추천을 위해 먼저 포트폴리오를 작성해주세요!"
        );
      } else {
        ...
        return (
          <div>
            ...
            <div>{renderCards(recommendedData)}</div>
          </div>
        );
      }
    } else {
        ...
        <div>
          <Row gutter={16}>
            {recommendedCard ? (
              ...
              <h2>
                추천 기능을 사용하시려면, 정확한 추천을 위해 먼저 포트폴리오를
                작성해주세요!
              </h2>
            )}
          </Row>
  ...
  return (
    <div>
      ...
      >
        <div style={{ position: "absolute", zIndex: 2, width: "55%" }}>
          {relatedSearchTermEnable
            ? renderSection("User", searchData.userSearchDtoList)
            : null}
        </div>
      </div>
      ...
      >
        <Row>
          <Col span={18} style={{ textAlign: "left" }}>
            ...
            <Button
              type={
                location.pathname === "/portfoliocard" ? "primary" : "default"
              }
            ...
            <Button
              type={location.pathname === "/project" ? "primary" : "default"}
              onClick={handleProjectPage}
            >
              Project
            </Button>
            <Button
              type={location.pathname === "/study" ? "primary" : "default"}
      ...
      >
        <div>
          <Button onClick={() => handleReload()}>전체 보기</Button>
        </div>
        <div>
          <Button onClick={() => handleRecommend()}>팀원 추천</Button>
        </div>
        <div style={{ marginLeft: "20px" }}>⬅️ 팀원을 추천받아 보세요!</div>
      </div>
      <div style={{ display: "grid" }}>{renderContent()}</div>

...
function MyPage() {
  ...
  const [isPasswordUpdateButtonEnabled, setIsPasswordUpdateButtonEnabled] =
    useState(false); //앞선 세 가지 필드가 다 입력되어야 비밀번호 변경 버튼이 활성화됨

  //회원 탈퇴 관련
  const [currentPasswordForSignOut, setCurrentPasswordForSignOut] =
    useState(""); //기존 비밀번호를 입력하는 필드
  ...
  useEffect(() => {
    ...
    const changePasswordFieldFilled =
      ...
      if (isConfirm) {
        request("PUT", "/updateUserInfo", updatedData)
          ...
          .catch((error) => {
            ...
            message.error(
              "정보 업데이트 중 오류가 발생했습니다. 나중에 다시 시도해주세요."
            );
          });
      } else {
  ...
  const updatePassword = () => {
    if (newPassword === confirmNewPassword) {
      //새로운 비밀번호와 새로운 비밀번호 확인 값이 같을 경우에만 백엔드 요청 보냄

      const passwordData = {
        //백엔드에 보낼 dto객체 생성 ->  UserPasswordUpdateDto(백엔드)
  ...
  const onSignOutHandler = () => {
    const signOutPasswordData = {
      //백엔드에 보낼 dto객체 생성 ->  signOuDto(백엔드)
              ...
              {userBaseInfo && (
                <Form>
                      ...
                      <div style={{ display: "flex", marginBottom: "8px" }}>
                        ...
                        {!remove && selectedImage ? (
                          ...
                          <Image
                            ...
                            onClick={() =>
                              handlePreview(URL.createObjectURL(selectedImage))
                  ...
                  <div style={{ marginTop: "10px" }}>
                    <Button
                      type="primary"
                      onClick={() => updateInfo(userBaseInfo)}

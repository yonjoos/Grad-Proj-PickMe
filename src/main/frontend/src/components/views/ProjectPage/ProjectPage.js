...
function ProjectPage() {
  ...
  const renderPosts = (posts) => {
    return (
      <div
        style={{ marginTop: "10px", padding: "1px", width: "100%" }}
        justify="space-between"
      >
        <Card
          title={`PROJECTS`}
          headStyle={{ background: "#fffec1", color: "#ff8400" }}
        >
          {posts.map((item, index) => (
            <div>
              <div style={{ display: "flex", marginTop: "0px" }}>
                ...
                >
                  <div>
                    모집 인원 &nbsp; {item.counts} / {item.recruitmentCount}{" "}
                    <br></br>모집 기한 &nbsp; {formatDate(item.endDate)}{" "}
                    <br></br> 조회 수 &nbsp;&nbsp;&nbsp;&nbsp; {item.viewCount}
                    <br />
                    <br />
      ...
      >
        <div style={{ position: "absolute", zIndex: 2, width: "55%" }}>
          {relatedSearchTermEnable
            ? renderSection("Project", searchData.projectSearchDtoList)
            : null}
        </div>
      </div>
      ...
      <Row>
        <Col span={12} style={{ textAlign: "left", margin: "0 0" }}>
          ...
          <Button
            ...
            onClick={handlePortfolioCardPage}
          >
            Portfolios
          </Button>
          <Button

...
import { formatDate, formatDateTime } from "../../../common/common";
...
function DetailProjectPage() {
  ...
  const [commentEditConfirmModalVisible, setCommentEditConfirmModalVisible] =
    useState(false); // 댓글, 답글 수정 관련 모달 활성화 여부
  const [isTopLevelUsedByEditing, setIsTopLevelUsedByEditing] = useState(null); // 댓글, 답글 수정 시 해당 댓글 또는 답글이 최상위 부모인지 아닌지 판단하는 기준값
  const [
    commentDeleteConfirmModalVisible,
    setCommentDeleteConfirmModalVisible,
  ] = useState(false); // 댓글, 답글 삭제 관련 모달 활성화 여부
  ...
  const handleModalConfirm = async () => {
    ...
    if (modalAction === "delete") {
      try {
        ...
        // 지원한 인원들이 모두 지원을 취소해야 비로소 삭제 가능. 이 부분은 정책 검토 필요.
        message.warning(
          "프로젝트를 삭제하려면 승인했던 인원을 모두 승인 취소해주세요."
        );
      }
    }
  ...
  const handleApproveUser = async (applyUserNickName, projectId) => {
    try {
      ...
      const response = await request(
        "PUT",
        `/posts/detail/approve?${queryParams}`
      );
  ...
  const handleCancelApproval = async (applyUserNickName, projectId) => {
    try {
      ...
      const response = await request(
        "PUT",
        `/posts/detail/cancelApprove?${queryParams}`
      );
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const response = await request(
        "POST",
        `/registerCommentsInProject/${projectId}`,
        {
  ...
  const loadMoreComments = () => {
    ...
    if (newPageSize >= totalPages) {
      // 만약 limit+3값이 전체 댓글보다 크거나 같다면 더보기 버튼 더 가져올 수 없고, 마지막 댓글로 limit값 설정
    ...
    return comments.map((comment) => (
      <div className={`comment-container depth-${depth}`}>
          ...
          {comment.commentWriter && (
            <div style={{ display: "flex", textAlign: "right" }}>
              ...
              <Button
                ...
                style={{ color: "black" }}
                onClick={() =>
                  showCommentDeleteConfirmModal(comment.id, comment.isTopLevel)
                }
              >
                삭제
              </Button>
          ...
          <>
            ...
            >
              ...
              <Button
                size="small"
                onClick={() =>
                  showCommentEditConfirmModal(comment.id, comment.isTopLevel)
                }
              >
                수정 완료
              </Button>
        ...
        {replyToCommentId === comment.id && ( // 답글 달기 버튼 누른 부모 댓글 아래에 답글 작성할 폼 세팅
          <div
            className={`reply-container depth-${depth + 1}`}
            style={{
              display: "flex",
              alignItems: "center",
              marginTop: "5px",
          ...
          >
            ...
            <p style={{ marginRight: "10px" }}>
              <strong>{currentUserNickName}</strong>
            </p>
            <TextArea
              autoSize={{ minRows: 3 }}
              type="text"
  ...
  const renderApplicantButton = () => {
    return (
      <div>
        ...
        <div>
          {isApplicantOpen &&
            applicantData.length > 0 && ( // applicantData가 비어있지 않을 때 렌더링
              <div>
                ...
                {isApplicantOpen && applicantData.length > 0 && (
                  ...
                  >
                    <div style={{ width: "100%", display: "flex" }}>
                      <Card
                        title={<span style={{ fontSize: "15px" }}>지원자</span>}
                        style={{ flex: 1 }}
                        headStyle={{
                          paddingLeft: "15px",
                          marginTop: "0px",
                              ...
                              <div key={index} style={{ marginTop: "15px" }}>
                                <Row>
                                  <Col
                                    span={16}
                                    onClick={() =>
                                      handleNickNameClick(applicant.nickName)
                                    }
                    ...
                    <div style={{ width: "100%", display: "flex" }}>
                      <Card
                        title={<span style={{ fontSize: "15px" }}>팀원</span>}
                        style={{ flex: 1 }}
                        headStyle={{
                          paddingLeft: "15px",
                          marginTop: "0px",
        ...
        <Col span={12} style={{ textAlign: "right" }}>
          ...
          {isWriter && (
            <div style={{ marginTop: "10px", marginBottom: "10px" }}>
              ...
              <Button onClick={() => showModal("delete")}>게시물 삭제</Button>
            </div>
          )}
          ...
          {!isWriter &&
            ...
            (data.counts === data.recruitmentCount ? (
              ...
              <div style={{ marginTop: "10px", marginBottom: "10px" }}>
                <Button
                  type="primary"
                  onClick={() => showScrapModal("cancelScrap")}
                  style={{ marginRight: "5px" }}
                >
                  스크랩 취소
                </Button>
          ...
          {!isWriter &&
            ...
            (data.counts === data.recruitmentCount ? (
              ...
              <div style={{ marginTop: "10px", marginBottom: "10px" }}>
                ...
                <Button
                  type="primary"
                  onClick={() => showCancelModal("applying")}
                >
                  지원 취소
                </Button>
              </div>
          ...
          {!isWriter && !isScrapped && isApplied && (
            <div style={{ marginTop: "10px", marginBottom: "10px" }}>
              ...
              <Button
                type="primary"
                onClick={() => showCancelModal("approved")}
              >
                지원 취소
              </Button>
            </div>
  ...
  const renderPost = (data) => {
    return (
      <div style={{ marginTop: "20px" }}>
        <Card>
          <div
            style={{ display: "grid", marginLeft: "10px", marginRight: "10px" }}
          >
            <div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{ display: "grid" }}>
                  ...
                  >
                    {data.web && (
                      <span
                        style={{ ...categoryTagStyle, background: "#fffec1" }}
                      >
                        #WEB
                      </span>
                    )}
                ...
                <div style={{ alignItems: "center" }}>
                  ...
                  <div style={{ color: "gray" }}>
                    {formatDateTime(data.finalUpdatedTime)}
                  </div>
                </div>
              </div>
              <hr></hr>
              <div>
                {data.fileUrl && data.fileUrl.length >= 1 ? (
                  <Card
                    style={{ borderborderRadius: "0px", border: "none" }}
                    size="small"
                    title={`첨부파일`}
                    bodyStyle={{
                      paddingTop: "0px",
                    ...
                    headStyle={{ borderRadius: "0px", background: "#fffec1" }}
                  >
                    {data.fileUrl
                      ? data.fileUrl.map((file, index) => (
                          ...
                          >
                            <Button
                              ...
                              onClick={() =>
                                window.open(
                                  `https://storage.googleapis.com/hongik-pickme-bucket/${file.fileUrl}`,
                                  "_blank"
                                )
                              } // 파일 열기 함수 호출
                            >
              ...
              {areCommentsVisible && (
                <div>
                  ...
                  <div style={{ textAlign: "right", marginTop: "16px" }}>
                    <Button size="small" onClick={handleCommentSubmit}>
                      등록
                    </Button>
                  </div>
                  {/* <div style={{ textAlign: 'center', margin: '20px 0' }}>
      ...
      {data.writer !== null ? (
        ...
        <div>
          ...
          {!data.writer &&
            ...
            !data.applied &&
            renderButtons()}{" "}
          {/** 지원 O 승인 X인 사람 (승인 대기 중) + 스크랩 안한 사람 */}
          {!data.writer &&
            data.scrap &&
            data.applying &&
            !data.applied &&
            renderButtons()}{" "}
          {/** 지원 O 승인 X인 사람 (승인 대기 중) + 스크랩 한 사람 */}
          {!data.writer &&
            !data.scrap &&
            !data.applying &&
            data.applied &&
            renderButtons()}{" "}
          {/** 승인 O인 사람 (승인 완료) + 스크랩 안한 사람 */}
          {!data.writer &&
            data.scrap &&
            !data.applying &&
            data.applied &&
            renderButtons()}{" "}
          {/** 승인 O인 사람 (승인 완료) + 스크랩 한 사람 */}
          {renderPost(data)}
          {/* Modal */}
          <Modal
            ...
            cancelText="아니오"
          >
            {modalAction === "delete" && <p>게시물을 삭제하시겠습니까?</p>}
            {modalAction === "apply" && <p>게시물에 지원하시겠습니까?</p>}
          </Modal>
          <Modal
            title="Confirm Action"
          ...
          <Modal
            ...
            cancelText="아니오"
          >
            {scrapAction === "scrap" && <p>게시물을 스크랩하시겠습니까?</p>}
            {scrapAction === "cancelScrap" && <p>스크랩을 취소하시겠습니까?</p>}
          </Modal>

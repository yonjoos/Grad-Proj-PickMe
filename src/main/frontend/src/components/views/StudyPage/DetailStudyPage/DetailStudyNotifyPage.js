...
function DetailStudyNotifyPage() {
                ...
                {applicantData.map((applicant, index) => (
                  <div key={index}>
                    ...
                    <Row>
                      <Col
                        span={8}
                        onClick={() => handleNickNameClick(applicant.nickName)}
                      ...
                      >
                        <Button
                          size="small"
                          onClick={() =>
                            handlePortfolioClick(applicant.nickName)
                          }
                        >
                          포트폴리오
                        </Button>
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between" }}>
      ...
      <div style={{ flex: 1 }}>{/** 좌측 여백을 위해 만든 더미 div */}</div>
      ...
      <div style={{ flex: 2.5 }}>
        ...
        >
          <Col span={16}>
            <div style={{ marginLeft: "5%" }}>제목: {data.title}</div>
          </Col>
        ...
        <div style={{ marginLeft: "5px" }}>첨부 파일: {data.fileUrl}</div>
      ...
      <div style={{ flex: 1 }}>{data.writer && renderApplicantButton()}</div>
    </div>
  );
}
